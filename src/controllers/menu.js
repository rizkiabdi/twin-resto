...
export  async function getMenuByCategory(req,res,next){
    try {
        ...
        const [rows] =  await connection.query(`SELECT m.name,m.price FROM menu as m INNER JOIN category_menu  c on m.category_id = c.id LEFT JOIN  WHERE c.name = '${categoryName}'`)
